...
async function check(proxies) {
    const res = await Promise.all(proxies.map((proxy) => {
        const agent = new socks_proxy_agent_1.SocksProxyAgent(`socks5://${proxy}`, { timeout: 10000 });
    ...
    if (res.length > 0) {
        valid += res.length;
        (0, fs_1.appendFileSync)("socks5.txt", res.join("\n") + "\n");
    }
}
async function main() {
    const sources = [
    ...
    for (let source of sources) {
        const proxies = await axios_1.default
            .get(source)
